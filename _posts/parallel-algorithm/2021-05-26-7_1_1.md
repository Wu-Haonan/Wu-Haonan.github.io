...
**步骤 $1$** （并行计算） 对每个处理机 $l\in \left{ 1,2,\ldots,p \right}$ ，计算下面问题的一个近似解 $(y_{l}^{i},\lambda_{\overline{l}}^{i})\in \mathbb{R}^{n_{l}} \times \mathbb{R}^{p-1}$ :

$$
\begin{aligned}
\min_{(x_l,\lambda_{\overline{l}})} \psi_{l}^{i} (x_{l},\lambda_{\overline{l}}) &= f(x_{l},x_{\overline{l}}^{(i)} + D_{\overline{l}}^{i}\lambda_{\overline{l}}), \\
