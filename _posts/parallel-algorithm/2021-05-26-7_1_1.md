...
$$\min_{x\in \mathbb{R}^{n}} f(x) \label{7.1.1}$$
...
x^{i_{l}} &= (y_{l}^{i},x_{\overline{l}}^{(i)} + D_{\overline{l}}^{i}\lambda_{\overline{l}}^{i}). \label{7.1.2}
\end{aligned}
$$

**步骤 $2$** （同步运算） 计算 $x^{(i+1)}$ ，使其满足下面不等式

$$f(x^{(i+1)}) \leq \min_{l\in \{1,2,\ldots,p\}} f(x^{i_{l}}) = \psi_{l}^{i} (y_{l}^{i},\lambda_{\overline{l}}^{i}) \label{7.1.3}$$
