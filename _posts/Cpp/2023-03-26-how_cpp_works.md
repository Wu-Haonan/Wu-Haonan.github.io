...
The "Log" funtion take a string (called message) and print the string to the console. Here we can simply treat the "const char*" as a kind of type that can hold a string of text. Then we want to move the "Log" funtion to a new C++ file (called Log.cpp). 
...
Actually, we will get a lot of errors after we compile it, because we don't write "declaration" in Log.cpp to declarate "cout" funtion.
