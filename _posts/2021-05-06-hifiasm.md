...
什么叫做primary assembly呢，这一步的想达到什么结果嗯。前面已经已经粗略的了解primary assembly了，具体来讲，我们先尽可能的保证一组assembly的连续性，这个assembly的每个contig，都可以确定完全来自于一种单倍型，但是这个assembly中的所有contigs我们不能保证只来自于同一个单倍型，其实是在两个单倍型之间反复转换的。但是，我们又保证，每个contig对应的染色体的区域，其相应的另一条同源染色体的该区域不会存在于primary assembly中。如下图所示，上面生成的contig就被选入primary assembly集合中，而下面的序列作为同源区段，暂时被抛弃。

![Primary assembly](/post_image/hifiasm/primary-assembly.PNG)
__Fig.9 Primary assembly[^5].__
