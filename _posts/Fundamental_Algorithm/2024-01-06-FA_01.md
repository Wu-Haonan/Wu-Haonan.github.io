...
We know if we want to represent $a$ in binary expansion, we need $\log a$ bits, and we do $q$ times minus that means the total running time is $\mathcal{O}(q \log a)$.
