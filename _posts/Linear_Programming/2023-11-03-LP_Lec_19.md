...
Let $\bar{a}_1, ..., \bar{a}_m$ be the rows of $A$ and suppose that $\bar{a}_m = \sum_{i=1}^{m-1} \lambda_i \bar{a}_i$ for some scalars $\lambda_1, ..., \lambda_{m-1}$, that means the last equality constraint is redundant and can be eliminated. 
