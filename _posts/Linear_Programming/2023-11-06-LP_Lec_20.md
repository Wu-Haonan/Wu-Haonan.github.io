...
<p align="center">
    <img src="Wu-Haonan.github.io/blob/master/post_image/Linear_Programming/geo_view_dual.PNG" width="70%">
<p>

Consider a primal problem with two variables and five inequality constraints (n = 2, m = 5), and suppose that no two of the vectors $\bar{a}_i$ are collinear. Every two-element subset I of {1,2,3,4,5} determines basic solutions $\bar{x}'$ and $\bar{p}'$ of the primal and the dual, respectively.

- If $I = \{1,2\}$, $\bar{x}'$ is primal infeasible (point A) and $\bar{p}'$ is dual infeasible, because $c$ cannot be expressed as a nonnegative linear combination of the vectors $\bar{a}_1$ and $\bar{a}_2$.
- If $I = \{1,3\}$, $\bar{x}'$ is primal feasible (point B) and $\bar{p}'$ is dual infeasible.
- If $I = \{1,4\}$, $\bar{x}'$ is primal feasible (point C) and $\bar{p}'$ is dual feasible, because $c$ can be expressed as a nonnegative linear combination of the vectors $\bar{a}_1$ and $\bar{a}_4$. In particular, $\bar{x}'$ and $\bar{p}'$ are optimal.
- If $I = \{1,5\}$, $\bar{x}'$ is primal infeasible (point D) and $\bar{p}'$ is dual feasible.
