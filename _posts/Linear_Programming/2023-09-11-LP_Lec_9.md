...
Equality holds in $(\ast)$ **if and only if** $\bar{a}_i^{\top} \bar{x} = b_i, \forall i \in I_{\bar{x}^\*}$. However, since $\bar{x}^\*$ is a basic feasible solution, there are $n$ linearly independent constraints that are active at $\bar{x}^\*$ and $\bar{x}^\*$ is therefore the unique solution of the linear system $\bar{a}_i^{\top} \bar{x} = b_i, i \in I_{\bar{x}^\*}$ according to above [theorem](https://wu-haonan.github.io/2023/09/11/LP_Lec_9.html#active-constraints). $\square$
