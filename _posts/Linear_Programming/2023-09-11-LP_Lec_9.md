...
Suppose ${\text{span}\\{\bar{a}_i \mid i \in I^{n*}\\} = \mathbb{R}^n}$. Then, one can extract a basis for $\mathbb{R}^n$ from the set ${\\{\bar{a}_i \mid i \in I_{x^\*}\\}}$. The ${n}$ vectors that are constituting basis are necessarily linearly independent (by the definition of basis.)
...
Suppose the vectors $\bar{a}_i$ with $i \in I_{x^\*}$ **do not** span $\mathbb{R}^n$,  we can choose a nonzero vector ${\bar{d} \in (\text{span}\\{\bar{a}_i \mid i \in I_{x^\*}\\})^\perp}$. Then, we have $\bar{a}_i^{\top} x^\* = b_i,\forall i \in I_{x^\*}$ and ${\bar{d} \in (\text{span}\\{\bar{a}_i \mid i \in I^{n*}\\})^\perp \iff \bar{a}_i^{\top} \bar{d} = 0,\forall i \in I^{n*}}$.
