...
Since $\bar{x}^*$ is not a basic feasible solution, there do not exist $n$ linearly independent vectors in ${\\{ \bar{a}_i \mid i \in I\_{x^\*} \\}}$. Hence ${\text{span} \\{ \bar{a}_i \mid i \in I\_{x^\*} \\}}$ is a proper subspace of $\mathbb{R}^n$, and the dimension of ${(\text{span} \\{ \bar{a}_i \mid i \in I\_{x^\*} \\})^\perp}\geq 1$, and we can pick a nonzero vector $\bar{d} \in (\text{span} \{ \bar{a}_i \mid i \in I_{x^\*} \})^\perp$.
