...
Since $x^*$ is not a basic feasible solution, there do not exist $n$ linearly independent vectors in ${\\{ \bar{a}_i \mid i \in I\_{x^\*} \\}}$. Hence ${\text{span} \\{ \bar{a}_i | i \in I\_{x^\*} \\}}$ is a proper subspace of $\mathbb{R}^n$, and the dimension of ${(\text{span} \\{ \bar{a}_i | i \in I\_{x^\*} \\})^\perp}\geq 1$, and we can pick a nonzero vector $\bar{d} \in (\text{span} \{ \bar{a}_i | i \in I_{x^*} \})^\perp$.



In other words, there exists $\bar{d} \in \mathbb{R}^n$ such that $\bar{a}_\i^{\top} \bar{d} = 0, \forall i \in I\_{x^\*}$. Consider the points of the form $x^* + \epsilon \bar{d}$ where $\epsilon$ is a small positive number.
