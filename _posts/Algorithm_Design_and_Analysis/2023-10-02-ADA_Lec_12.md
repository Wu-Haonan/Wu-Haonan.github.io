...
Step 1: Solve the Dual LP, and get optimal solution ${ y\_1^\*, y\_2^\*,\cdots,  y\_n^\* }$ with value ${ Z_\D^\* }$

Step 2: Add ${ j }$ to solution ${ I }$ if the dual constraint for ${ S_j }$ is tight, that is ${ \sum_{i \in \{i\mid e_i \in S_j\}} y_\i^\* = w_j }$
...
2. Feasibility: Suppose element ${ e_i }$ is not covered. Hence, for all the ${ S_j }$ such that ${ e_i \in S_j }$, their corresponding dual constraints is not tight (otherwise, we will pick up it in Step 2). So, we can inrease ${ y\_i^\* }$ while keeping the solution still feasible. Which contradicts to ${ Z_\D^\* }$ is optimal value. 

3. ${ f }$-approximation: we pick ${ S_j, j\in I }$ because constraint is tight. So, we have ${ \sum_{j\in I} w_j = \sum_{j\in I} \sum_{i \in \{i\mid e_i \in S_j\}} y_\i^\* }$. Because, ${ e_i }$ appears at most in ${ f }$ subsets, so ${ \sum_{j\in I} w_j = \sum_{j\in I} \sum_{i \in \{i\mid e_i \in S_j\}} y_\i^\* \leq f \sum_{i =1} y\_i^\* = f Z\_D^\* = f Z\_{LP}^\* \leq f Z\_{ILP}^\* = f OPT }$