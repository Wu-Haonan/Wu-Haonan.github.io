...
RNA string ${ s_1,\cdots,s_n }$ over ${ \{A,C,G,U\} }$

it's tries to form base pairs with itself by broping back

Thes set of base pairs formed by RNA moleculars by this process is called secondary structure. 

is a set of pairs ${ S = \{ (s_i, s_j) \} }$
...
3. No crossing: If ${ (s_i,s_j), (s_k ,s_{\ell}) \in S}$ 
