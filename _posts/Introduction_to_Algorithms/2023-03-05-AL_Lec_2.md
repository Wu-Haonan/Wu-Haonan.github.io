...
\begin{equation}
\begin{aligned}
c_1\cdot n^{\log_b a} \cdot \sum_{i=0}^{\log_b n -1} (\lg n/b^i)^k &\geq c_1\cdot n^{\log_b a} \cdot \left(\lg b\right)^k \cdot {\log_b n} \cdot \left(\sqrt[\log_b n] {(\log_b n)!}\right)^k \\
&\geq c_1\cdot n^{\log_b a} \cdot \left(\lg b\right)^k \cdot {\log_b n} \cdot \left(\sqrt[\log_b n] {\sqrt {2\pi \log_b n} \left(\frac{\log_b n}{e}\right)^{\log_b n} e^{\frac{1}{12(\log_b n)+1}}}\right)^k \\
&= c_1\cdot n^{\log_b a} \cdot \left(\lg b\right)^k \cdot (\log_b n)^{k+1} \cdot \left(\frac{1}{e}\right)^k \cdot \left(\sqrt[\log_b n] {\sqrt {2\pi \log_b n} \cdot e^{\frac{1}{12(\log_b n)+1}}}\right)^k \\
&= c_1\cdot n^{\log_b a} \cdot \left(\lg b\right)^k \cdot (\log_b n)^{k+1} \cdot \left(\frac{1}{e}\right)^k \cdot \left(
{2\pi}^{\frac{1}{2 \log_b n}} \cdot (\log_b n)^{\frac{1}{2 \log_b n}} \cdot e^{\frac{1}{(12(\log_b n)+1)\cdot \log_b n}} \right)^k
\end{aligned}
\end{equation}
$$</center>

Here, it's trivial to check the below funtions are monotone decreasing (because ${n>1 \Rightarrow \log_b n >0 }$), and their limits are ${ 1 }$, which is also their lower bound.

<center>$$
{2\pi}^{\frac{1}{2 \log_b n}} \geq 1 , (\log_b n)^{\frac{1}{2 \log_b n}} \geq 1, e^{\frac{1}{(12(\log_b n)+1)\cdot \log_b n}} \geq 1
$$</center>
...
Combining the left and right part, in the end, we prove ${ g(n) = \Theta(n^{\log_b a} \cdot (\lg n)^{k+1}) }$.
