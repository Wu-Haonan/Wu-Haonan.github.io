...
<b>Case \#2</b>: Because ${ f(n)=\Theta(n^{\log_b a} (\lg n)^k), k\geq 0 }$, so there exists ${ c_1>0,c_2>0,n_0>0 }$ such that ${ c_1\cdot n^{\log_b a} (\lg n)^k \leq f(n) \leq c_2\cdot n^{\log_b a} (\lg n)^k }$, when ${ n>n_0 }$. Therefore, we can get 
...
&\geq c_1\cdot n^{\log_b a} \cdot \left(\lg b\right)^k \cdot (\log_b n)^k \\
&= c_1\cdot n^{\log_b a} \cdot \left(\lg b\right)^k \cdot \left(\frac{\lg n}{\lg b}\right)^k \\
&= c_1\cdot n^{\log_b a} \cdot \left(\lg n\right)^k
\end{aligned}
\end{equation}
$$</center>
